...
export class MasterIPC extends EventEmitter {
	...
	private _disconnect(message: NodeMessage) {
		const { d: id } = message.data;
		this._debug(`Cluster ${id} disconnected!`);
		this.manager.emit(SharderEvents.DISCONNECT, id);
	}
	...
	private _shardresume(message: NodeMessage) {
		...
		this.manager.emit(SharderEvents.SHARD_RESUME, shardID);
	}
